...
export function RouteOverlay({
  ...
  if (!layout.nodes.length || !reroutes.length) {
    return (
      <div
        className="flex flex-col items-center justify-center h-full min-h-[360px] gap-3 rounded-2xl"
        style={{
  ...
  const pad = 70;
  const baseW = Math.max(maxX - minX + pad * 2, 600);
  const baseH = Math.max(maxY - minY + pad * 2, 400);

  // Apply zoom factor to viewBox width & height centered
  ...
  const scaleRange = Math.min(baseW - pad * 2, baseH - pad * 2);
  const sx = (x: number) => ((x - minX) / rangeX) * scaleRange + pad;
  const sy = (y: number) => ((y - minY) / rangeY) * scaleRange + pad;

  const nodeMap = new Map(layout.nodes.map((n) => [n.id, n]));

  const handleZoomIn = () => setZoomScale((z) => Math.min(2.5, z + 0.25));
  const handleZoomOut = () => setZoomScale((z) => Math.max(0.5, z - 0.25));
  const handleResetZoom = () => setZoomScale(1.0);

  return (
    <div
      className="h-full min-h-[460px] rounded-2xl overflow-hidden flex flex-col relative"
      style={{
    ...
    >
      ...
      <div
        className="absolute top-3 right-3 z-10 flex items-center gap-1 p-1.5 rounded-xl shadow-lg"
        style={{
      ...
      >
        <button
          ...
          title="Zoom In"
          className="w-7 h-7 flex items-center justify-center rounded-lg text-xs font-bold text-slate-200 hover:bg-blue-600/30 hover:text-white transition-all"
        >
          🔍 +
        </button>
        <button
        ...
        <span className="text-[10px] font-mono font-semibold text-blue-400 px-1">
          {Math.round(zoomScale * 100)}%
        </span>
        <button
          onClick={handleResetZoom}
          title="Reset Zoom"
          className="w-7 h-7 flex items-center justify-center rounded-lg text-xs font-bold text-slate-300 hover:bg-blue-600/30 hover:text-white transition-all"
        >
          ↺
        </button>
      </div>
      ...
      >
        ...
        {/* Background */}
        <rect width={baseW * 2} height={baseH * 2} x={-baseW} y={-baseH} fill="url(#route-grid)" />
          ...
          return (
            <g key={node.id}>
              <circle
                ...
                stroke={isOnRoute ? "#ffffff" : "rgba(30,58,95,0.9)"}
                strokeWidth={isOnRoute ? 2.5 : 1}
              />
              <text
                x={x}
                y={y}
